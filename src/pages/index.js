...
import { useAllCountries, useCreateCountry } from '@/hooks/country-hook'
...
export default function Home() {
  ...
  const { data, isLoading, isFetching } = useAllCountries();
  ...
  console.log("ALL COUNTRIES ISLOADING ? ", isLoading);

  console.log("FETCH STATUS", isFetching);
//
  return (
    <main
      className={`flex min-h-screen flex-col items-center justify-between p-24 ${inter.className}`}
    >
      <Link href="/home"><h2>Go to next page</h2></Link>


      <CountryForm onSubmit={handleSubmit} />

      <h2 className='my-5 text-lg'>Total count: {data?.data?.length}</h2>
